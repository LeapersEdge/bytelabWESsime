...
static volatile bool _lock_state = false;

/* ===== Commands from PWA ===== */

static volatile bool locked = 0;
// #include "esp_timer.h"
static void cmd_toggle_lock(void)
{
    ...
    // if ((now_us - s_last_toggle_us) < 2000000) {
    //     return;
    // }
    // s_last_toggle_us = now_us;

    // bool locked = parental_lock_get_state();
...
static void cmd_save_schedule(const char *msg)
{
    ...
    if (matched == 2) {
        esp_err_t err = parental_lock_save_schedule(from_minutes, to_minutes);
        if (err != ESP_OK) {
...
{
    set_app_state(APP_STATE_INTERCOM);
...
esp_err_t https_server_start(void)
{
    ...
    // conf.max_uri_len = 2048;
    // conf.max_resp_headers = 2048;
