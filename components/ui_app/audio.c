        ...
        if (xQueueReceive(audio_queue, &req, portMAX_DELAY) == pdTRUE) {
            if (req.priority < current_priority || current_priority == 0) {
                ...
                switch (req.clip_id) {
                    ...
                    // Music
                    case 5:  clip = polka;             clip_len = polka_len;    is_music_clip = true; break;
                    case 6:  clip = elevator;          clip_len = elevator_len; is_music_clip = true; break;
                    case 7:  clip = elise;             clip_len = elise_len;    is_music_clip = true; break;
                ...
                if (is_music_clip) {
                    lv_obj_clear_flag(ui_MusicNoteImg1, LV_OBJ_FLAG_HIDDEN);
                    lv_obj_clear_flag(ui_MusicNoteImg2, LV_OBJ_FLAG_HIDDEN);
                }

                play_melody(clip, clip_len);

                if (is_music_clip) {
                    lv_obj_add_flag(ui_MusicNoteImg1, LV_OBJ_FLAG_HIDDEN);
                    lv_obj_add_flag(ui_MusicNoteImg2, LV_OBJ_FLAG_HIDDEN);
                }
